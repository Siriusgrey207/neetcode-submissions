class Solution {
    public boolean isValidSudoku(char[][] board) {
        // First we need to check each row. If there are repeating chars, we don't have a valid sudoku.
        ...
        // Then we search for repeating elements in each column.
        for (int col = 0; col < 9; col++) {
            Set<Character> seen = new HashSet<>();
            for (int row = 0; row < 9; row++) {
                if (board[row][col] == '.') continue;
        ...
        // Finally, we seach for repeating elements in each square.
